...
```bash
getUploadUrl$ sam local invoke helloFromLambdaFunction --no-event
```
...
Resources:
  ...
  helloFromLambdaFunction:
    ...
    Properties:
      CodeUri: ./src/getUploadUrl/
      Handler: getUploadUrl::getUploadUrl.Function::FunctionHandler
...
```bash
getUploadUrl$ sam logs -n helloFromLambdaFunction --stack-name sam-app --tail
```
...
```bash
getUploadUrl$ dotnet test test/getUploadUrl.Tests/getUploadUrl.Tests.csproj 
```
...
```bash
aws cloudformation delete-stack --stack-name getUploadUrl
```
