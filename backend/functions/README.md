...
```bash
cloudphotobackend$ sam local invoke helloFromLambdaFunction --no-event
```
...
Resources:
  ...
  helloFromLambdaFunction:
    ...
    Properties:
      CodeUri: ./src/cloudphotobackend/
      Handler: cloudphotobackend::cloudphotobackend.Function::FunctionHandler
...
```bash
cloudphotobackend$ sam logs -n helloFromLambdaFunction --stack-name sam-app --tail
```
...
```bash
cloudphotobackend$ dotnet test test/cloudphotobackend.Tests/cloudphotobackend.Tests.csproj 
```
...
```bash
aws cloudformation delete-stack --stack-name cloudphotobackend
```
